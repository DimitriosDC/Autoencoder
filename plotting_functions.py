...
def plot_grids(data, x_figures=5, y_figures=10, receptor_split=900, region_shape=30):
    ...
    subfigs = fig.subfigures(x_figures, y_figures) # create figure subpanels
    ...
    for outer_idx, subfig in enumerate(subfigs.flat): # for each suplot create inner figures
        ...
        if outer_idx < np.size(data,0): # check that there is enough data for the plot
            ...
            subfig.subplots_adjust(top=0.9) # shift plot
